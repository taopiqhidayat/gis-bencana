<?php
    ...
    $query = mysqli_query($knk, "SELECT id, incident as bencana, district as daerah, latitude as titu, longitude as ngitu, meninggal, luka, hilang, bangunan, rumah, lahan, kerugian, tgl FROM disaster ORDER BY tgl ASC");
    $data = array();
    $x = 0;
    ...
    while($rw = mysqli_fetch_assoc($query)) {
        if ($rw["titu"] != null || $rw["ngitu"] != null) {
            $data[$x] = [
                ...
                "titu" => $rw["titu"],
                "ngitu" => $rw["ngitu"],
                "keterangan" => "Meninggal: " . $rw["meninggal"] .
                                " Luka-luka: " . $rw["luka"] .
                                " Hilang: " . $rw["hilang"] .
    ...
    $query = mysqli_query($knk, "SELECT * FROM reports_disaster ORDER BY tanggal ASC");
    $datare = array();
    $y = 0;
    ...
    while($re = mysqli_fetch_assoc($query)) {
        $datare[$y] = [
            ...
            "ngitu" => $re["longitude"],
            "tanggal" => $re["tanggal"],
            "keterangan" => "Meninggal: " . $re["wafat"] .
                            " Luka-luka: " . $re["luka"] .
                            " Hilang: " . $re["hilang"] .
                            " Bangunan: " . $re["bangunan"] .
                            " Rumah: " . $re["rumah"] .
                            " Lahan: " . $re["lahan"] .
                            " Kerugian: " . $re["kerugian"],
    ...
    if (isset($_POST["terima"])) {
        if (terReIncident($_POST) > 0) {
            echo "<script>
                    alert('Anda telah Berhasil menerima laporan bencana!');
                </script>";
        } else {
...
<body>
    <div class="container">
        ...
        <div class="card mt-3 mb-5">
            <div class="card-body">
                <table id="tabrebencana" class="table table-striped" style="width: 100%;">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Nama Pelapor</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= $nik ?></td>
                            <td><?= $nohp ?></td>
                            <td><?= $datare[$e]["insiden"] ?></td>
                            <td><?= $datare[$e]["alamat"] ?></td>
        ...
        <a href="create_incident.php" class="btn btn-primary mt-3">Tambah Data</a>

        <div class="card mt-3">
            <div class="card-body">
                <table id="tabbencana" class="table table-striped" style="width: 100%;">
                    <thead>
                        <tr>
                            <th>No</th>
                            <th>Bencana</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><?= $data[$a]["daerah"] ?></td>
                            <td><?= $data[$a]["titu"] . ", " . $data[$a]["ngitu"] ?></td>
                            <td><?= $data[$a]["tgl"] ?></td>
                            <td><?= $data[$a]["keterangan"] ?></td>
                            <?php if ($_SESSION["user"] === 1) : ?>
                            <td>
                                <a href="update_incident.php?id=<?= $data[$a]["id"] ?>" class="btn btn-warning">Ubah</a>
                            </td>
                            <?php endif; ?>
                        </tr>
                        <?php endfor; ?>
    ...
    <script>
        $(document).ready(function () {
            $('#tabbencana').DataTable();
            $('#tabrebencana').DataTable();
        });
    </script>
</body>
</html>